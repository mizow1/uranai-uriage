...
実行時の対話フロー：
1. 設定ファイルで指定された保存場所の現在年月のlineディレクトリから統合対象CSVファイルを自動検出・表示（抽出日付も表示）
2. 出力ファイル名を指定（デフォルト: `line-menu-YYYY-MM.csv`）
3. 既存ファイルがある場合は上書き確認
4. 最終実行確認

統合処理の詳細：
- **統合対象**: `YYYY-MM-DD_コンテンツ名.csv`形式のファイル
- **除外ファイル**: `output.csv`、`line-menu-*.csv`、`line-contents-*.csv`
- **ヘッダー処理**: 最初のファイルのヘッダー行のみ採用し、先頭に「日付」列を追加
- **データ処理**: 各CSVファイルの2行目以降（データ行）を統合、各行に日付を追加
- **保存場所**: 統合対象ファイルと同じディレクトリ（`base_path/YYYY/YYYYMM/line/`）
...
├── YYYY/           # 年フォルダ
    ├── YYYYMM/     # 月フォルダ
        ...
        │   ├── YYYY-MM-DD_元のファイル名.csv    # 個別CSVファイル
...
### ファイル種類
- **個別CSVファイル**: `YYYY-MM-DD_元のファイル名.csv` - lineサブディレクトリに保存
...
### v2.2の主な変更点
- **CSV統合機能の追加**: `--merge-csvs`オプションで設定ファイル指定のlineディレクトリ内のCSVファイルを統合
...
- **メール処理との連携**: メール処理完了後の自動CSV統合確認機能
- **ヘッダー行処理**: CSVファイルのヘッダー行を正しく処理し重複を排除
...
### v2.3の主な変更点
- **ファイル名形式の変更**: 個別CSVファイルの命名規則を`元のファイル名_YYYY-MM-DD.csv`から`YYYY-MM-DD_元のファイル名.csv`に変更
- **CSV統合機能の対応**: 新しいファイル名形式に対応したCSV統合機能の更新
- **日付抽出機能の強化**: 新旧両方のファイル名形式から日付を正確に抽出する機能を実装
- **後方互換性**: 旧形式のファイルも引き続き処理可能
